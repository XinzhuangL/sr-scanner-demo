...
#ifndef BE_HTTP_CLIENT_H
#define BE_HTTP_CLIENT_H
...
namespace starrocks {
    ...
    public:
        ...
        void set_http_header(const std::vector<std::string>& http_header) {
            std::vector<std::string>::const_iterator iter = http_header.begin();
            while (iter != http_header.end()) {
                _header_list = curl_slist_append(_header_list, iter -> c_str());
                iter ++;
            }
            curl_easy_setopt(_curl, CURLOPT_HTTPHEADER, _header_list);
        }
        ...
        // helper function to download a file, you can call this fucntion to download
